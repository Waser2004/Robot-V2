...
struct Context {
    ...
    size_t target_rotation_buffer_size    = 5; // max size of the target rotation buffer
    float  target_rotation_buffer[5][6];
    int    target_rotation_buffer_amount  = 0; // amount of rotations in the buffer

    // rotation deadzone (angles in which the actuator cannot move)
    float  actuator_deadzone_start[6]   = {315, 135, 225, 135, 45,  135};
    float  actuator_deadzone_end[6]     = {225, 225, 315, 225, 135, 225};
    

    // actuator angles
    float  actuator_zero_pos[6]           = {50.5f, 238.9746, 28.125, 147.832, 353.3203, 3.515625}; // raw sensor value that corresponds to actuator 0 rotation
    ...
    // settings
    float  actuator_rotation_tolerance    = 0.5; // min offset between real and target rotation in degrees per actuator to be considered as "reached"
};